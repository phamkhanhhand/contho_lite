...
namespace CT.TagHelpers
{
    ...
    [HtmlTargetElement("ct-combobox")]
    public class CTComboboxTagHelper : CTBaseTagHelper
    {
...
<script>
    ...
    App[" + htmlIDStr + @"] = temp;
        ...
        private void AddScriptAndCss(TagHelperOutput output)
        {
            ...
            output.PostElement.AppendHtml("<link rel='stylesheet' href='/css/ct-combobox.css' />");

            // Thêm script (ví dụ: validate input số, ngày tháng)
            output.PostElement.AppendHtml("<script src='/js/ct-combobox.js'></script>");
