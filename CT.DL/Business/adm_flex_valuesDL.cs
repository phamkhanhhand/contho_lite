...
namespace CT.DL
{
    public class adm_flex_valuesDL : BaseDL
    {
        ...
        public bool AddLink(ParentChildDTO linkSetDTO)
        {
            ...
            var sql = @"[dbo].[proc_save_hierarchy_value_all_set]";

            // Tạo DataTable cho TVP
            DataTable scopeTableChild = new DataTable();
            scopeTableChild.Columns.Add("value", typeof(string));

            for (int i = 0; i < linkSetDTO.ListChild?.Count; i++)
            {
                scopeTableChild.Rows.Add(linkSetDTO.ListChild[i]);
            }

            DataTable scopeTableParent = new DataTable();
            scopeTableParent.Columns.Add("value", typeof(string));

            for (int i = 0; i < linkSetDTO.ListParent?.Count; i++)
            {
                scopeTableParent.Rows.Add(linkSetDTO.ListParent[i]);
            }
            ...
            cmd.Parameters.AddWithValue("@id", linkSetDTO.id);
